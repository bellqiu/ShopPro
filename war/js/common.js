...
if(typeof(jQuery)!='undefined') {
	(function($) {
		...
		/**
		 * 左右滚动容器中的li元素.html结构为<div>
		 *                         <ul>
		 *                            <li></li>
		 *                            <li></li>
		 *                         </ul>
		 *                       </div>
		 * @param config Object配置对象{btnPrev:'向左的按钮ID',btnNext:'向右的按钮ID',speed:'移动间隔时间(毫秒)','step':'每次移动的距离(px)','auto':'是否自动滚动(boolean),'autoSlipTime':'自动滚动的时间间隔(毫秒),'autoSlipDir':'自动滚动时的方向,默认向右',默认5000','count':'每行显示个数,默认4个'}
		 */
